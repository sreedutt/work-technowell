-- Function: generate_taxdetails(text, text, text)

-- DROP FUNCTION generate_taxdetails(text, text, text);

CREATE OR REPLACE FUNCTION generate_taxdetails(iward text, izone text, iproperty_no text)
  RETURNS void AS
$BODY$
declare
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION generate_taxdetails(text, text, text)
  OWNER TO postgres;
