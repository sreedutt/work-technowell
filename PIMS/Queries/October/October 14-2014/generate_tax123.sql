...
CREATE OR REPLACE FUNCTION generate_tax123(iward text, izone text, iproperty_no text)
  RETURNS void AS
$BODY$
...
begin
	...
	loop
		...
		if(vocc_status = '1') then
			...
			if(vproperty_id = vproperty_id and vusage_type = '1' and vconst_class in ('A','B','C','D')) then
				...
				update flat_details_test set rate_sqm = vrate_res, rate_deprciate = vrate_depr where property_id = vproperty_id;
				...
				vmaint := sum(valv * 0.1);
				vrv := valv - vmaint;
				update flat_details_test set alv = valv, alv_op = 0, tenpc_maint = vmaint, rateable_value = vrv where property_id = vproperty_id;  
			...
			elsif(vproperty_id = vproperty_id and (vusage_type = '2' or vusage_type = '3' or vusage_type = '7') and vconst_class in ('A','B','C','D')) then
				select rate_commercial from rate_master into vrate_com where zone = vzone and const_class = vconst_class;
				select depreciation_pc from ratedepr_master into vrate_depr where const_class = vconst_class and age_factor = vage_factor;
				update flat_details_test set rate_sqm = vrate_com, rate_deprciate = vrate_depr where property_id = vproperty_id;
				select sum(assess_area * rate_sqm)  from flat_details_test into alv1 where property_id = vproperty_id;
				select sum(alv1 * (rate_deprciate/100)) from flat_details_test into alv2 where property_id = vproperty_id;
				valv := sum((alv1-alv2) * 12);
				vmaint := sum(valv * 0.1);
			...
			elsif(vproperty_id = vproperty_id and (vusage_type = '2' or vusage_type = '3' or vusage_type = '7') and vconst_class = 'E') then
				select rate_commercial from rate_master into vrate_com where zone = vzone and const_class = vconst_class;
			--select depreciation_pc from ratedepr_master into vrate_depr where const_class = vconst_class and age_factor = vage_factor;
			...
			if(vproperty_id = vproperty_id and vusage_type = '1' and vconst_class in ('A','B','C','D')) then
				...
				if(valv > vannualrent) then
					...
					update flat_details_test set alv = vannualrent, alv_op = 0, tenpc_maint = vmaint, rateable_value = vrv where property_id = vproperty_id;
				end if;
